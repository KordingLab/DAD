function Results = DAD(Ytest,Xtrain,C,Xtest,Ttest,Ttrain)
...
%if length(idx1)>1 && length(idx2)>1 && LP(10)>C.EigRatio*max(LP)
...
Xnm = normc(Xtrain-repmat(mean(Xtrain),size(Xtrain,1),1));
Fnm = normc(F-repmat(mean(F),size(F,1),1));
[YrKLF, ~, KLD,~, ~,~,RM,sconst] = minKL(Fnm,Xnm,C);
...
subplot(2,3,2); colorData2014(YrKL,Ttest(idx2)); title('YrKL')
subplot(2,3,3); colorData2014(YLo2,Ttest(idx2)); title('YLo2')
subplot(2,3,4); colorData2014(Xtest,Ttest); title('Xtest')
subplot(2,3,5); colorData2014(Xtrain,Ttrain); title('Xtrain')
