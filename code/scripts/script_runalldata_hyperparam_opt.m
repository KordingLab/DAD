...
% training set
[YtrJ,TtrJ,XtrJ] = compile_jango_neuraldata(whichtarg,numS,numD,'binnedData_0801.mat');

% test set (1)
[YteJ2,TteJ2,XteJ2] = compile_jango_neuraldata(whichtarg,numS,numD,'binnedData_0807.mat');
YteJ2 = downsamp_nd(YteJ2,2);

% test set (2)
[YteJ3,TteJ3,XteJ3] = compile_jango_neuraldata(whichtarg,numS,numD,'binnedData_0819.mat');
YteJ3 = downsamp_nd(YteJ3,2);

[YteJ4,TteJ4,XteJ4] = compile_jango_neuraldata(whichtarg,numS,numD,'binnedData_0901.mat');
...
if iscell(XrecM)
    num = length(XrecM);
    R2_alldata = zeros(5,num);
    KL_alldata = zeros(5,num);
    for i=1:num 
    ...
    R2_alldata(1) = evalR2(XteM(:,1:2),XrecM); 
    R2_alldata(2) = evalR2(XtrC0(:,1:2),XrecC); 
    R2_alldata(3) = evalR2(XteJ2(:,1:2),XrecJ2); 
    R2_alldata(4) = evalR2(XteJ3(:,1:2),XrecJ3); 
    R2_alldata(5) = evalR2(XteJ4(:,1:2),XrecJ4);
    
    KL_alldata(1) = min(minKLM(:));
    KL_alldata(2) = min(minKLC(:));
    KL_alldata(3) = min(minKLJ2(:));
    KL_alldata(4) = min(minKLJ3(:)); 
    KL_alldata(5) = min(minKLJ4(:));
