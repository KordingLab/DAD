function [R2, minVal, Results] = calcR2_minKL3Dgrid(V,X,Xgt,A,K,Methods,numsol)
%%%%%%%%
% Required Input:
...
for i = 1:numA
    ...
    for j=1:numK
        for jj = 1:numV
            ...
            Res = minKL_grid3D(V{jj},X,avec,svec,'L2',K(j),numsol); 
            ...
            tmpr2 = R2Vf(:,i,j,jj);
            [R2max,maxID] = max([R2X(i,j,jj), tmpr2(:)', R2V(i,j,jj), R2Vicp(i,j,jj)]);
